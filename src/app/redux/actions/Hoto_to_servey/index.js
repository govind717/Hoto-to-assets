...
export const hoto_servey_data_disptach = function ({ page = 1,search_value = "" } = {}) {
    return async (dispatch) => {
        try {
            ...
            const login_response = await axios.post('https://survey.lumacorp.in/api/survey-auth/login?email=abishek@gmail.com&password=password')
            const user_details = login_response?.data;
            const servey_data_response = await axios.get(`https://survey.lumacorp.in/api/survey/get_gp_survey_details?page=${page}&search=${search_value ? search_value : ""}`, {
                headers: {
