...
import { oandm_warehouse_replacement_request_assign_data_disptach } from "app/redux/actions/O&M/Warehouse";
...
const ReplacementAssignRequest = () => {
  ...
  const { oandmWarehouseReplacementRequestAssignDataReducer } = useSelector(
    (state) => state
  );
  ...
  const handleSearch = (searchTerm) => {
    setPage(1);
    dispatch(
      oandm_warehouse_replacement_request_assign_data_disptach({
        sortBy: sortBy,
  ...
  return (
    <>
      {oandmWarehouseReplacementRequestAssignDataReducer?.loading && (
        <FullScreenLoader />
      )}
