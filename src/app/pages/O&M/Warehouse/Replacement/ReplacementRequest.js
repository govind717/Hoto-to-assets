import { Button, InputAdornment, Pagination, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TableSortLabel, TextField } from "@mui/material";
...
import { oandm_warehouse_replacement_request_data_disptach } from "app/redux/actions/O&M/Warehouse";
...
const ReplacementRequest = () => {
 ...
 const [openGeneratePickupRequest,setOpenGeneratePickupRequest]=useState(false);
  const { oandmWarehouseReplacementRequestDataReducer } = useSelector(
    (state) => state
  );
  ...
  const handleSearch = (searchTerm) => {
    setPage(1);
    dispatch(
      oandm_warehouse_replacement_request_data_disptach({
        sortBy: sortBy,
    ...
    <>
      ...
      <TableContainer sx={{ marginTop: "15px" }} component={Paper}>
        <Table sx={{ minWidth: 650 }} size="small">
          ...
          <TableBody>
            {oandmWarehouseReplacementRequestDataReducer?.data?.result?.data
              ?.length > 0 ? (
              oandmWarehouseReplacementRequestDataReducer?.data?.result?.data?.map(
                (ele, index) => {
                  return (
                    <TableRow key={ele?.id}>
                      <TableCell
        ...
        <Pagination
          count={
            oandmWarehouseReplacementRequestDataReducer?.data?.result
              ?.total_pages || 1
          }
          page={page}
