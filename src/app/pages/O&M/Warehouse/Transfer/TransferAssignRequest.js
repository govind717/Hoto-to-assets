...
import { oandm_warehouse_transfer_request_assign_data_disptach } from "app/redux/actions/O&M/Warehouse";
...
const TransferAssignRequest = () => {
  ...
  const handleSearch = (searchTerm) => {
    setPage(1);
    dispatch(
      oandm_warehouse_transfer_request_assign_data_disptach({
        sortBy: sortBy,
  ...
  return (
    <>
      {oandmWarehouseTransferRequestAssignDataReducer?.loading && (
        <FullScreenLoader />
      )}
      ...
      <TableContainer sx={{ marginTop: "15px" }} component={Paper}>
        <Table sx={{ minWidth: 650 }} size="small">
          ...
          <TableBody>
            {oandmWarehouseTransferRequestAssignDataReducer?.data?.result?.data
              ?.length > 0 ? (
              oandmWarehouseTransferRequestAssignDataReducer?.data?.result?.data?.map(
                (ele, index) => {
                  return (
                    <TableRow key={ele?.id}>
                      <TableCell
        ...
        <Pagination
          count={
            oandmWarehouseTransferRequestAssignDataReducer?.data?.result
              ?.total_pages || 1
          }
          page={page}
