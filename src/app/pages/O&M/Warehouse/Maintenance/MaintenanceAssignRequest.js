...
import { oandm_warehouse_maintenace_request_assign_data_disptach } from "app/redux/actions/O&M/Warehouse";
...
const MaintenanceAssignRequest = () => {
  ...
  const { oandmWarehouseMaintenaceRequestAssignDataReducer } = useSelector(
    (state) => state
  );
  ...
  const handleSearch = (searchTerm) => {
    setPage(1);
    dispatch(
      oandm_warehouse_maintenace_request_assign_data_disptach({
        sortBy: sortBy,
  ...
  return (
    <>
      {oandmWarehouseMaintenaceRequestAssignDataReducer?.loading && (
        <FullScreenLoader />
      )}
