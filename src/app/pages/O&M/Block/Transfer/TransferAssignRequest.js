    ...
    <>
      ...
      <TableContainer sx={{ marginTop: "15px" }} component={Paper}>
        <Table sx={{ minWidth: 650 }} size="small">
          ...
          <TableBody>
            {oandmBlockTransferRequestAssignDataReducer?.data?.result?.data
              ?.length > 0 ? (
              oandmBlockTransferRequestAssignDataReducer?.data?.result?.data?.map(
                (ele, index) => {
                  return (
                    <TableRow key={ele?.id}>
                      <TableCell
        ...
        <Pagination
          count={
            oandmBlockTransferRequestAssignDataReducer?.data?.result
              ?.total_pages || 1
          }
          page={page}
