...
import { hoto_gp_asset_partfolio_transfer_data_disptach } from "app/redux/actions/Hoto_to_servey/GP";
...
const TranferTable = ({row}) => {
  ...
  const { hotoGpAssetPortfolioTransferDataReducer } = useSelector(
    (state) => state
  );
  ...
  const handleSearch = (searchTerm) => {
    setPage(1);
    dispatch(
      hoto_gp_asset_partfolio_transfer_data_disptach({
        sortBy: sortBy,
    ...
    <>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} size="small">
          ...
          <TableBody>
            {hotoGpAssetPortfolioTransferDataReducer?.data?.result?.data
              ?.length > 0 ? (
              hotoGpAssetPortfolioTransferDataReducer?.data?.result?.data?.map(
                (ele, index) => {
                  return (
                    <TableRow key={ele?.id}>
                      <TableCell
